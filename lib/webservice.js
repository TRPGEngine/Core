...
function WebService(opts) {
  ...
  if(!fs.existsSync(publicDir)) {
    ...
    appLogger('create public dir at:', publicDir);
...
function initRoute() {
  ...
  if(!!this.homepage) {
    ...
    appLogger('set webserver homepage to:' + this.homepage);
  }else {
    router.get('/', (ctx) => {
  ...
  for (var apiPath in this.webApi) {
    ...
    appLogger('register web api [%s] success!', apiPath);
  }
...
WebService.prototype.listen = function() {
  ...
  appLogger('start to listen(%d)', this.port);
