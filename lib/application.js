...
app.request = {
  get(url, query, config) {
    return axios(Object.assign({}, {
      ...
      applog('[request GET]', url, query, res);
      return res.data;
    })
  },
  post(url, data, config) {
    return axios(Object.assign({}, {
      ...
      applog('[request POST]', url, data, res);
      return res.data;
    })
  }
}
    ...
    storage.resetAsync(async function(db) {
      try {
        if(!!app.events['resetStorage']) {
          ...
          console.log('registered reset event completed!');
        }
        console.log('sending resetStorage to modules');
        // app.emit('resetStorage', storage, db); // deprecated
      }catch(err) {
