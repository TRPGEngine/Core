...
app.initReport = function initReport() {
  try {
    let app = this;
    this.report = new Report(app.get('report'));
      ...
      socket.on(eventName, (data, cb) => {
        ...
        event.fn.call(wrap, data, function(res) {
          ...
          if(res.result === false) {
            ...
            app.report.reportError(eventName, '-->', res); // 使用汇报服务汇报错误
          }else {
...
app.test = async function test() {
  ...
  let client = {
    ...
    emit(...args) {
      return new Promise((resolve, reject) => {
        socket.emit(...args, function(data) {
          if(data.result) {
            resolve(data);
          }else {
            reject(data.msg);
  ...
  console.log(`测试结果: 共${counts.all}条测试 成功/失败: ${counts.passed} 条 /${counts.failed} 条`);
}
