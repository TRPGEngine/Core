...
app.initWebService = function initWebService() {
  try {
    ...
    this.webservice = new WebService({port});
    debug("create webservice(%d) success!", port);
  }catch(err) {
    debug("create webservice error:%O", err);
    throw err;
  }finally {
    return this;
  }
...
app.initIO = function initIO() {
  try {
    ...
    if(this.webservice) {
      ...
      this.io = IO(this.webservice.getHttpServer());
    }else {
...
app.initIOEvent = function initIOEvent() {
  ...
  app.io.on('connection', function(socket) {
    ...
    for (let event of app.socketEvents) {
      socket.on(event.name, event.fn.bind(wrap));
    }
  });
...
app.registerEvent = function(eventName, eventFn) {
  ...
  this.socketEvents.push({
    ...
    fn: function (data, cb) {
      ...
      if(verbose) {
        debugSocket("[%s]%s <-- %o", socketId, eventName, JSON.parse(JSON.stringify(data)));
      }else {
        debugSocket("%s <-- %o", eventName, JSON.parse(JSON.stringify(data)));
      }

      eventFn.call(this, data, function(res) {
        cb(res);
        if(verbose) {
          debugSocket("[%s]%s --> %o", socketId, eventName, JSON.parse(JSON.stringify(res)));
        }else {
          debugSocket("%s --> %o", eventName, JSON.parse(JSON.stringify(res)));
        }
      })
    }
  });
...
app.reset = function reset() {
  ...
  if(storage) {
    ...
    storage.reset(async function(db) {
      try {
